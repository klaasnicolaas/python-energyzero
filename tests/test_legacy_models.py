"""Test Legacy models for EnergyZero API."""
...
############################################################################
##  Test EnergyZero models                                                ##
############################################################################


@pytest.mark.freeze_time("2022-12-07 15:00:00+01:00")
async def test_legacy_electricity_model(
    ...
    energy: Electricity = await energyzero_client.get_electricity_prices_legacy(
        start_date=today,
        end_date=today,
...
async def test_legacy_electricity_none_date(
    aresponses: ResponsesMockServer,
    snapshot: SnapshotAssertion,
    energyzero_client: EnergyZero,
) -> None:
...
async def test_legacy_electricity_midnight_cest(
    aresponses: ResponsesMockServer,
    snapshot: SnapshotAssertion,
    energyzero_client: EnergyZero,
) -> None:
...
async def test_legacy_no_electricity_data(
    ...
    aresponses.add(
        ...
        aresponses.Response(
            status=200,
            headers={"Content-Type": "application/json"},
            text=load_fixtures("legacy/no_data.json"),
        ),
    )
    today = date(2022, 12, 7)
    with pytest.raises(EnergyZeroNoDataError):
        await energyzero_client.get_electricity_prices_legacy(
            start_date=today, end_date=today
        )


@pytest.mark.freeze_time("2022-12-07 15:00:00+01:00")
async def test_legacy_gas_model(
    aresponses: ResponsesMockServer,
    snapshot: SnapshotAssertion,
    energyzero_client: EnergyZero,
) -> None:
    ...
    aresponses.add(
        ...
        aresponses.Response(
            status=200,
            headers={"Content-Type": "application/json"},
            text=load_fixtures("legacy/gas.json"),
        ),
    )
    today = date(2022, 12, 7)
    gas: Gas = await energyzero_client.get_gas_prices_legacy(
...
async def test_legacy_gas_morning_model(
    aresponses: ResponsesMockServer,
    snapshot: SnapshotAssertion,
    energyzero_client: EnergyZero,
) -> None:
...
async def test_legacy_gas_none_date(
    aresponses: ResponsesMockServer,
    snapshot: SnapshotAssertion,
    energyzero_client: EnergyZero,
) -> None:
...
async def test_legacy_no_gas_data(
    aresponses: ResponsesMockServer, energyzero_client: EnergyZero
) -> None:
    """Raise exception when there is no data."""
    aresponses.add(
    ...
    today = date(2022, 12, 7)
    with pytest.raises(EnergyZeroNoDataError):
        await energyzero_client.get_gas_prices_legacy(start_date=today, end_date=today)
